...
export default function Stocks() {
  ...
  const selectSymbol = (s: string) => {
    setSymbol(s); setSearch(''); setSuggestions([]); setShowSuggestions(false)
  }

  const handleSearch = (e: React.FormEvent) => {
    e.preventDefault()
    if (suggestions.length > 0) selectSymbol(suggestions[0].symbol)
    else if (search.trim()) selectSymbol(search.trim().toUpperCase())
  }
  ...
  return (
    <div className="p-8">
          ...
          <button key={s} onClick={() => selectSymbol(s)}
            className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${
              symbol === s
                ...
                : 'bg-white dark:bg-slate-700 border border-slate-200 dark:border-slate-600 text-slate-600 dark:text-slate-300 hover:border-emerald-300'
            }`}>{s}</button>
        ))}
      </div>
      ...
      {quoteLoading ? (
        ...
        <>
          <Card className="mb-6">
            ...
            <div className="grid grid-cols-2 gap-4 mt-6 pt-6 border-t border-slate-100 dark:border-slate-700">
              ...
              <div>
                <p className="text-xs text-slate-400">{t('stocks.change')}</p>
        ...
        <p className="text-slate-400 text-sm">{t('stocks.notFound')}</p>
      )}
    </div>
  )
}