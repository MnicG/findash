  ...
  return (
    <div className="p-4 md:p-8">
      <div className="mb-6 md:mb-8">
        <h1 className="text-xl md:text-2xl font-bold text-slate-800 dark:text-slate-100">{t('news.title')}</h1>
        <p className="text-slate-500 dark:text-slate-400 mt-1 text-sm">{t('news.subtitle')}</p>
      </div>

      <form onSubmit={(e) => { e.preventDefault(); setSubmitted(query.trim()) }} className="flex gap-2 md:gap-3 mb-6 md:mb-8">
        ...
        {submitted && (
          <button type="button" onClick={() => { setQuery(''); setSubmitted('') }}
            className="border border-slate-200 dark:border-slate-600 text-slate-600 dark:text-slate-300 px-3 md:px-4 py-2 rounded-lg text-sm font-medium hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors shrink-0">
            {t('news.clear')}
          </button>
        )}
      </form>
      ...
      {loading ? (
        <div className="space-y-4">
          {[...Array(5)].map((_, i) => (
            <div key={i} className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-4 md:p-6 animate-pulse">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-3 md:gap-4">
          {articles.filter(a => a.title !== '[Removed]').map((article, i) => (
            <a key={i} href={article.url} target="_blank" rel="noopener noreferrer"
              className="block bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-4 md:p-6 hover:shadow-md hover:border-emerald-200 dark:hover:border-emerald-700 transition-all group">
